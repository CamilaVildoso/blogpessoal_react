...
function CardTemas({tema}: CardTemaProps) {
  return (
    <div className='border flex flex-col rounded-2xl overflow-hidden justify-between'>
      <header className='py-2 px-6 bg-amber-500 text-white font-bold text-2xl'>Tema</header>
      <p className='p-8 text-3xl bg-slate-100 h-full'>{tema.descricao}</p>
      <div className="flex">
        <Link to={`/editarTema/${tema.id}`} className='w-full text-slate-100 bg-amber-400 hover:bg-amber-600 flex items-center justify-center py-2'>
